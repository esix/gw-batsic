...
(~1,000 assertions, see the `*.test.bat` files). The numbered deep-dive
...
- 10 — Arrays (`DIM`, indexing, multi-dim, read/write & `READ`/`INPUT` into elements). ✅ *implemented* (`OPTION BASE` not yet)
- 11 — Operators (`+ - * / ^`, integer `\` and `MOD`, comparisons, boolean `AND`/`OR`/`NOT`/`XOR`/`EQV`/`IMP`). ✅ *implemented*
- 12 — Numeric built-ins (`ABS`/`INT`/`SGN`/`FIX`/`SQR`/`SIN`/`COS`/`TAN`/`ATN`/`LOG`/`EXP`/`RND`/`CINT`/`CSNG`/`CDBL`/`TIMER`). ✅ *implemented* (`INSTR`/`HEX$`/`OCT$` still pending)
- 13 — String built-ins (`LEN`/`CHR$`/`STR$`/`VAL`/`ASC`/`LEFT$`/`RIGHT$`/`MID$` func & statement/`SPACE$`/`STRING$`). ✅ *implemented*
- 14 — `PRINT` formatting (`PEND`/`PSEMI`/`PTAB`/`PZONE`, `TAB(`/`SPC(`), `PRINT USING`, `LPRINT`. ✅ *implemented*
...
- 18 — **File I/O** — handle table, sequential `OPEN`/`CLOSE`/`PRINT #`/`WRITE #`/`INPUT #`/`EOF`/`LOF`, random-access `FIELD`/`LSET`/`RSET`/`GET`/`PUT` live-window records, `MKI$`…`CVD` codecs, `FILES`/`KILL`/`NAME`. ✅ *implemented*
