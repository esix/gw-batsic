...
:ErrorCodeToString code ret
  ...
  if "!code!"=="1" set "ret=NEXT without FOR"
  ...
  if "!code!"=="2" set "ret=Syntax error"
  ...
  if "!code!"=="6" set "ret=Overflow"
  rem The ret of a calculation is too large to be represented in GW-BASIC's 
  rem number format. If underflow occurs, the ret is zero, and execution 
  rem continues without an error.

  if "!code!"=="7" set "ret=Out of memory"
  ...
  if "!code!"=="10" set "ret=Duplicate Definition"
  ...
  if "!code!"=="11" set "ret=Division by zero"
  rem A division by zero is encountered in an expression, or the operation of 
  rem involution rets in zero being raised to a negative power. Machine 
  rem infinity with the sign of the numerator is supplied as the ret of the 
  rem division, or positive machine infinity is supplied as the ret of the 
  rem involution, and execution continues.

  if "!code!"=="12" set "ret=Illegal direct"
  rem A statement that is illegal in direct mode is entered as a direct mode 
  rem command.

  if "!code!"=="13" set "ret=Type mismatch"
  ...
  if "!code!"=="14" set "ret=Out of string space"
  ...
  if "!code!"=="22" set "ret=Missing operand"
  rem An expression contains an operator with no operand following it.

  if "!code!"=="23" set "ret=Line buffer overflow"
  rem An attempt is made to input a line that has too many characters.

  if "!code!"=="24" set "ret=Device Timeout"
  rem GW-BASIC did not receive information from an I/O device within a 
  rem predetermined amount of time.

  if "!code!"=="25" set "ret=Device Fault"
  rem Indicates a hardware error in the printer or interface card.

  if "!code!"=="26" set "ret=FOR Without NEXT"
  rem A FOR was encountered without a matching NEXT.

  if "!code!"=="27" set "ret=Out of Paper"
  rem The printer is out of paper; or, a printer fault.

  if "!code!"=="28" set "ret=Unprintable error"
  rem No error message is available for the existing error condition. This is 
  rem usually caused by an error with an undefined error code.

  if "!code!"=="29" set "ret=WHILE without WEND"
  rem A WHILE statement does not have a matching WEND.

  if "!code!"=="30" set "ret=WEND without WHILE"
  rem A WEND was encountered without a matching WHILE.

  if "!code!"=="31" set "ret=Unprintable error"
  if "!code!"=="32" set "ret=Unprintable error"
  if "!code!"=="33" set "ret=Unprintable error"
  if "!code!"=="34" set "ret=Unprintable error"
  if "!code!"=="35" set "ret=Unprintable error"
  if "!code!"=="36" set "ret=Unprintable error"
  if "!code!"=="37" set "ret=Unprintable error"
  if "!code!"=="38" set "ret=Unprintable error"
  if "!code!"=="39" set "ret=Unprintable error"
  if "!code!"=="40" set "ret=Unprintable error"
  if "!code!"=="41" set "ret=Unprintable error"
  if "!code!"=="42" set "ret=Unprintable error"
  if "!code!"=="43" set "ret=Unprintable error"
  if "!code!"=="44" set "ret=Unprintable error"
  if "!code!"=="45" set "ret=Unprintable error"
  if "!code!"=="46" set "ret=Unprintable error"
  if "!code!"=="47" set "ret=Unprintable error"
  if "!code!"=="48" set "ret=Unprintable error"
  if "!code!"=="49" set "ret=Unprintable error"
  rem No error message is available for the existing error condition. This is 
  rem usually caused by an error with an undefined error code.

  if "!code!"=="50" set "ret=FIELD overflow"
  rem A FIELD statement is attempting to allocate more bytes than were specified
  rem for the record length of a random file.

  if "!code!"=="51" set "ret=Internal error"
  ...
  if "!code!"=="52" set "ret=Bad file number"
  ...
  if "!code!"=="55" set "ret=File already open"
  ...
  if "!code!"=="56" set "ret=Unprintable error"
  ...
  if "!code!"=="57" set ret="Device I/O Error"
  ...
  if "!code!"=="58" set "ret=File already exists"
  rem The filename specified in a NAME statement is identical to a filename 
  rem already in use on the diskette.

  if "!code!"=="59" set "ret=Unprintable error"
  if "!code!"=="60" set "ret=Unprintable error"
  rem No error message is available for the existing error condition. This is 
  rem usually caused by an error with an undefined error code.

  if "!code!"=="61" set "ret=Disk full"
  rem All disk storage space is in use.

  if "!code!"=="62" set "ret=Input past end"
  ...
  if "!code!"=="63" set "ret=Bad record number"
  ...
  if "!code!"=="64" set "ret=Bad filename"
  ...
  if "!code!"=="65" set "ret=Unprintable error"
  rem No error message is available for the existing error condition. This is 
  rem usually caused by an error with an undefined error code.

  if "!code!"=="66" set "ret=Direct statement in file"
  rem A direct statement is encountered while loading a ASCII-format file. The 
  rem LOAD is terminated.

  if "!code!"=="67" set "ret=Too many files"
  ...
  if "!code!"=="68" set "ret=Device Unavailable"
  ...
  if "!code!"=="69" set "ret=Communication buffer overflow"
  ...
  if "!code!"=="70" set "ret=Permission Denied"
  ...
  if "!code!"=="71" set "ret=Disk not Ready"
  ...
  if "!code!"=="72" set "ret=Disk media error"
  ...
  if "!code!"=="75" set "ret=Path/File Access Error"
  ...
  if "!code!"=="76" set "ret=Path not found"
