...
:init
  ...
  @REM (0xB4-0xB6 not used in GW-BASIC 3.23)
  set "_k_B7=WRITE"  & set "_c_WRITE=B7"
  set "_k_B8=OPTION" & set "_c_OPTION=B8"
  set "_k_B9=RANDOMIZE"& set "_c_RANDOMIZE=B9"
  set "_k_BA=OPEN"   & set "_c_OPEN=BA"
  ...
  set "_k_C1=MOTOR"  & set "_c_MOTOR=C1"
  set "_k_C2=BSAVE"  & set "_c_BSAVE=C2"
  set "_k_C3=BLOAD"  & set "_c_BLOAD=C3"
  set "_k_C4=SOUND"  & set "_c_SOUND=C4"
  set "_k_C5=BEEP"   & set "_c_BEEP=C5"
  set "_k_C6=PSET"   & set "_c_PSET=C6"
  set "_k_C7=PRESET" & set "_c_PRESET=C7"
  set "_k_C8=SCREEN" & set "_c_SCREEN=C8"
  set "_k_C9=KEY"    & set "_c_KEY=C9"
  set "_k_CA=LOCATE" & set "_c_LOCATE=CA"

  @REM --- Sub-keywords 0xCC-0xDE (0xCB unused) ---
  set "_k_CC=TO"     & set "_c_TO=CC"
  set "_k_CD=THEN"   & set "_c_THEN=CD"
  set "_k_CE=TAB("   & set "_c_TAB(=CE"
  set "_k_CF=STEP"   & set "_c_STEP=CF"
  set "_k_D0=USR"    & set "_c_USR=D0"
  set "_k_D1=FN"     & set "_c_FN=D1"
  set "_k_D2=SPC("   & set "_c_SPC(=D2"
  set "_k_D3=NOT"    & set "_c_NOT=D3"
  set "_k_D4=ERL"    & set "_c_ERL=D4"
  set "_k_D5=ERR"    & set "_c_ERR=D5"
  set "_k_D6=STRING$"& set "_c_STRING$=D6"
  set "_k_D7=USING"  & set "_c_USING=D7"
  set "_k_D8=INSTR"  & set "_c_INSTR=D8"
  @REM 0xD9 is the '-shorthand comment marker (REM apostrophe); not a user keyword,
  @REM so its code is here for the binary converter but no _c_ entry is exposed.
  set "_k_D9='"
  set "_k_DA=VARPTR" & set "_c_VARPTR=DA"
  set "_k_DB=CSRLIN" & set "_c_CSRLIN=DB"
  set "_k_DC=POINT"  & set "_c_POINT=DC"
  ...
  @REM token names (GT, EQ, LT, PLUS, MINUS, MUL, DIV, POW, IDIV) but those
  @REM aren't keywords the user can type as identifiers, so we don't put them
  @REM in `_c_*` (which would make isKeyword report true for "PLUS" etc.).
  @REM The binary converter uses `_k_XX` for byte→name lookups and a parallel
  @REM op-name→byte table.
  set "_k_E6=GT"
  set "_k_E7=EQ"
  set "_k_E8=LT"
  set "_k_E9=PLUS"
  ...
  @REM --- Logical / modulo keywords 0xEE-0xF3 ---
