@echo off
@REM CHAIN MERGE file[,line] : load/overlay another program, PRESERVING variables
...
set "_final=!%_s%!"
endlocal & set "%~1=%_final%" & set "_run_file=%_path%" & set "_run_line=%_chln%" & set "_chain_keep=1" & set "_chain_merge=1" & exit /B 98
:_ch_err
set "_final=!%_s%!"
